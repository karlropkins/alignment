...
#' warping (COW). This is \code{\link{cow}} but with argument structure like
...
#' \code{output}, can be used to modify this behavior, see also
#' \code{\link{alignment}}.
#' @note This function is based on previous matlab (see Source),
#' but some formal arguments and parameters have been changed to
#' make function more consistent with other \code{alignment} functions.
